...
__device__ float Distance(const Point_3 &a, const Point_3 &b)
{
    float dist = 0;
...
__device__ void SearchAtNode(const CUDA_KDNode *nodes, const int *indexes, const Point_3 *pts, int cur, const Point_3 &query, int *ret_index, float *ret_dist, int *ret_node)
{
...
__device__ void SearchAtNodeRange(const CUDA_KDNode *nodes, const int *indexes, const Point_3 *pts, const Point_3 &query,int cur, float range, int *ret_index, float *ret_dist)
{
...
__device__ void Search(const CUDA_KDNode *nodes, const int *indexes, const Point_3 *pts, const Point_3 &query, int *ret_index, float *ret_dist)
{
...
__global__ void SearchBatch(const CUDA_KDNode *nodes, const int *indexes, const Point_3 *pts, int num_pts, Point_3 *queries, int num_queries, int *ret_index, float *ret_dist)
{
...
void CUDA_KDTree::CreateKDTree(KDNode *root, int num_nodes, const vector <Point_3> &data)
{
    ...
    cudaMalloc((void**)&m_gpu_points, sizeof(Point_3)*m_num_points);
    ...
    cudaMemcpy(m_gpu_points, &data[0], sizeof(Point_3)*data.size(), cudaMemcpyHostToDevice);

    CheckCUDAError("CreateKDTree");
}

void CUDA_KDTree::Search(const vector <Point_3> &queries, vector <int> &indexes, vector <float> &dists)
{
    int threads = 512;
    ...
    cudaMalloc((void**)&gpu_queries, sizeof(Point_3)*queries.size()*KDTREE_DIM);
